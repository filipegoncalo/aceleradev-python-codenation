...
def classify_by_phone_number(records):
    ...
    for record in records:
        ...
        call_start = record['start']
        call_end = record['end']

        result = calculate_fare(call_start, call_end)

        taxed_numbers[phone_number] = round((taxed_numbers.setdefault(phone_number, 0) + result), 2)

    records = [{'source': record, 'total': taxed_numbers[record]} for record in taxed_numbers]
    records.sort(reverse=True, key=lambda tax: tax['total'])

    return records

def calculate_fare(call_start, call_end):
    rate = 0
    total = 0
    start = call_start
    end = call_end
    ...
    rate = checking_fare(start)

    if rate == DAY_RATE:
        total = (duration * DAY_RATE) + FIXED
    else:
        total = FIXED

    return total

def checking_fare(start):
    tax = 0
    call_start = datetime.fromtimestamp(start)

    if call_start.hour >= 22 or call_start.hour < 6:
        tax = FIXED
    else:
        tax = DAY_RATE
